...
{% macro priority_reject_reason(col) %}
    case
        when {{ col }} is null or trim(cast({{ col }} as varchar)) = '' then 'priority bị rỗng hoặc NULL'
        when try_cast({{ col }} as integer) is not null then 'priority là số ngoài khoảng 1..4'
        else 'priority là chuỗi không hợp lệ'
    end
{% endmacro %}
