...
def write_batch(con: duckdb.DuckDBPyConnection, batch: list[dict]) -> None:
    ...
    Câu lệnh hiện tại là INSERT ON CONFLICT DO UPDATE để đảm bảo tính idempotent.
    """
    con.begin()
    con.executemany(
        f"""
...
def consume(
    ...
    with LogConsumer(topic, offset_file) as consumer:
        ...
        while True:
            ...
            # Ghi dữ liệu trước (at-least-once), sau đó mới commit offset.
            # write_batch idempotent xử lý replay an toàn khi restart.
